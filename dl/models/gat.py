...
class Net(nn.Module):
    def __init__(self, args):
        ...
        h_feat = 256
        num_heads = 4
        embed_edge_feat = True
        e_in_feats = len(feat_dict[args.edge_feat_cols])
        e_out_feat1 = e_out_feat2 = 64

        self.layer1 = MultiHeadGATLayer(in_feats, h_feat, num_heads,
                                        embed_edge_feat=embed_edge_feat, e_in_dim=e_in_feats, e_out_dim=e_out_feat1)
        # Be aware that the input dimension is hidden_dim*num_heads since
        # multiple head outputs are concatenated together. Also, only
        # one attention head in the output layer.
        self.layer2 = MultiHeadGATLayer(h_feat * num_heads, num_classes, 1,
                                        embed_edge_feat=embed_edge_feat, e_in_dim=e_out_feat1*num_heads,
                                        e_out_dim=e_out_feat2)
    ...
    def forward(self, g):
        ...
        h, e_h = self.layer1(g, node_feat, edge_feat)
        h, e_h = F.elu(h), F.elu(e_h)
        h, e_h = self.layer2(g, h, e_h)
        return h, info
...
class MultiHeadGATLayer(nn.Module):
    ...
    def forward(self, g, h, e_h):
        head_outs = [attn_head(g, h, e_h) for attn_head in self.heads]
        node_outs, edge_outs = zip(*head_outs)
        if self.merge == 'cat':
...
class GATLayer(nn.Module):
    def __init__(self, in_dim, out_dim, embed_edge_feat, e_in_dim, e_out_dim):
        ...
        if self.embed_edge_feat:
            ...
            # equation (2)
            self.attn_fc = nn.Linear(2 * out_dim + 2, 1, bias=False)
